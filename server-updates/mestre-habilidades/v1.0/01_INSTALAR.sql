...
END IF;
IF EXISTS (SELECT 1 FROM _ika_hab_teachers e LEFT JOIN spell_template s ON s.Id=e.Id WHERE s.Id IS NULL OR NOT (s.`Effect1`=e.`Effect1` AND s.`Effect2`=e.`Effect2` AND s.`Effect3`=e.`Effect3` AND s.`EffectTriggerSpell1`=e.`EffectTriggerSpell1` AND s.`EffectTriggerSpell2`=e.`EffectTriggerSpell2` AND s.`EffectTriggerSpell3`=e.`EffectTriggerSpell3`)) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: efeito de ensino ausente ou diferente do diagnostico. Nada foi instalado.';
END IF;
IF (SELECT COUNT(*) FROM spell_template WHERE Id IN (71,355,688,691,697,712,883,982,1066,1122,1515,2458,2641,2842,3599,5149,5394,5487,5502,5784,6795,6807,6991,7328,7386,8071,8946,10140,13819,18540,18960,20252,23161,23214,28272,33388,33391,34767,34769,40120)) <> 40 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: uma habilidade final nao existe em spell_template. Nada foi instalado.';
END IF;
IF EXISTS (SELECT 1 FROM _ika_hab_links e LEFT JOIN spell_learn_spell s ON s.entry=e.entry AND s.SpellID=e.SpellID WHERE s.entry IS NULL OR s.Active<>e.Active) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: ensino complementar de venenos, ajudante ou montaria difere do diagnostico.';
END IF;
IF EXISTS (SELECT 1 FROM _ika_hab_sources e LEFT JOIN quest_template s ON s.entry=e.entry WHERE s.entry IS NULL OR s.RewSpellCast<>e.RewSpellCast) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: recompensa de missao de origem mudou. Nada foi instalado.';
END IF;
IF (SELECT COUNT(*) FROM item_template WHERE entry IN (5175,5176,5177,5178) AND maxcount=1 AND stackable=1 AND bonding=1 AND SellPrice=0)<>4 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: um item de totem difere do diagnostico. Nada foi instalado.';
END IF;
IF EXISTS (SELECT 1 FROM creature WHERE id=919600) AND NOT EXISTS (SELECT 1 FROM creature_template WHERE Entry=919600) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: existem spawns sem template com o ID escolhido. Nada foi instalado.';
END IF;
START TRANSACTION;
INSERT INTO `quest_template` SELECT e.* FROM `_ika_hab_quests` e LEFT JOIN `quest_template` t ON t.`entry`=e.`entry` WHERE t.`entry` IS NULL;
INSERT INTO `creature_questrelation` SELECT e.* FROM `_ika_hab_start` e LEFT JOIN `creature_questrelation` t ON t.`id`=e.`id` AND t.`quest`=e.`quest` WHERE t.`id` IS NULL;
INSERT INTO `creature_involvedrelation` SELECT e.* FROM `_ika_hab_end` e LEFT JOIN `creature_involvedrelation` t ON t.`id`=e.`id` AND t.`quest`=e.`quest` WHERE t.`id` IS NULL;
INSERT INTO `creature_template` SELECT e.* FROM `_ika_hab_npc` e LEFT JOIN `creature_template` t ON t.`Entry`=e.`Entry` WHERE t.`Entry` IS NULL;
IF EXISTS (SELECT 1 FROM `_ika_hab_npc` e LEFT JOIN `creature_template` t ON t.`Entry`=e.`Entry` WHERE t.`Entry` IS NULL OR NOT ((t.`Entry` <=> e.`Entry`) AND
        (BINARY t.`Name` <=> BINARY e.`Name`) AND
        (BINARY t.`SubName` <=> BINARY e.`SubName`) AND
        (BINARY t.`IconName` <=> BINARY e.`IconName`) AND
        (t.`MinLevel` <=> e.`MinLevel`) AND
        ...
        (t.`DamageMultiplierOLD` <=> e.`DamageMultiplierOLD`) AND
        (t.`DamageVarianceOLD` <=> e.`DamageVarianceOLD`))) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: validacao final falhou em creature_template. Preserve a saida e envie.';
END IF;
IF EXISTS (SELECT 1 FROM `_ika_hab_quests` e LEFT JOIN `quest_template` t ON t.`entry`=e.`entry` WHERE t.`entry` IS NULL OR NOT ((t.`entry` <=> e.`entry`) AND
        (t.`Method` <=> e.`Method`) AND
        (t.`ZoneOrSort` <=> e.`ZoneOrSort`) AND
        (t.`MinLevel` <=> e.`MinLevel`) AND
        (t.`MaxLevel` <=> e.`MaxLevel`) AND
        ...
        (t.`StartScript` <=> e.`StartScript`) AND
        (t.`CompleteScript` <=> e.`CompleteScript`))) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: validacao final falhou em quest_template. Preserve a saida e envie.';
END IF;
IF EXISTS (SELECT 1 FROM `_ika_hab_start` e LEFT JOIN `creature_questrelation` t ON t.`id`=e.`id` AND t.`quest`=e.`quest` WHERE t.`id` IS NULL OR NOT ((t.`id` <=> e.`id`) AND
        (t.`quest` <=> e.`quest`))) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: validacao final falhou em creature_questrelation. Preserve a saida e envie.';
END IF;
IF EXISTS (SELECT 1 FROM `_ika_hab_end` e LEFT JOIN `creature_involvedrelation` t ON t.`id`=e.`id` AND t.`quest`=e.`quest` WHERE t.`id` IS NULL OR NOT ((t.`id` <=> e.`id`) AND
        (t.`quest` <=> e.`quest`))) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='IKA: validacao final falhou em creature_involvedrelation. Preserve a saida e envie.';
END IF;
COMMIT;
SELECT 'INSTALACAO CONCLUIDA: NPC 919600 e 33 licoes conferidos. Reinicie mangosd.' AS resultado;
