-- IKA: verificacao somente de leitura das tabelas persistentes.
SET NAMES utf8;
USE tbcmangos;
...
(9737, 34766),
(11001, 40123);
SELECT 'creature_template' AS tabela, COUNT(*) AS esperados, SUM(t.`Entry` IS NOT NULL AND ((t.`Entry` <=> e.`Entry`) AND
        ...
        (t.`DamageVarianceOLD` <=> e.`DamageVarianceOLD`))) AS identicos FROM `_ika_hab_npc` e LEFT JOIN `creature_template` t ON t.`Entry`=e.`Entry`;
SELECT 'quest_template' AS tabela, COUNT(*) AS esperados, SUM(t.`entry` IS NOT NULL AND ((t.`entry` <=> e.`entry`) AND
        ...
        (t.`CompleteScript` <=> e.`CompleteScript`))) AS identicos FROM `_ika_hab_quests` e LEFT JOIN `quest_template` t ON t.`entry`=e.`entry`;
SELECT 'creature_questrelation' AS tabela, COUNT(*) AS esperados, SUM(t.`id` IS NOT NULL AND ((t.`id` <=> e.`id`) AND
        (t.`quest` <=> e.`quest`))) AS identicos FROM `_ika_hab_start` e LEFT JOIN `creature_questrelation` t ON t.`id`=e.`id` AND t.`quest`=e.`quest`;
SELECT 'creature_involvedrelation' AS tabela, COUNT(*) AS esperados, SUM(t.`id` IS NOT NULL AND ((t.`id` <=> e.`id`) AND
        (t.`quest` <=> e.`quest`))) AS identicos FROM `_ika_hab_end` e LEFT JOIN `creature_involvedrelation` t ON t.`id`=e.`id` AND t.`quest`=e.`quest`;
SELECT Entry,Name,SubName,Faction,NpcFlags,DisplayId1 FROM creature_template WHERE Entry=919600;
SELECT entry,Title,MinLevel,RequiredClasses,RequiredRaces,RequiredSkill,RequiredSkillValue,RewSpellCast,RewItemId1,SpecialFlags FROM quest_template WHERE entry IN (SELECT entry FROM _ika_hab_quests) ORDER BY RequiredClasses,MinLevel,entry;
SELECT guid,id,map,position_x,position_y,position_z FROM creature WHERE id=919600;
